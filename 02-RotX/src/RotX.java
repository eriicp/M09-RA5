public class RotX{
        public static String minuscules = "aáàbcçdéèfghiíìïjklmnñoóòpqrstuúùüvwxyz";
        public static String majuscules = "AÁÀBCÇDÉÈFGHIÍÌÏJKLMNÑOÓÒPQRSTUÚÙÜVWXYZ";
        public static char[] mArray = minuscules.toCharArray();
        public static char[] MArray = majuscules.toCharArray();
    public static void main(String[] args) {
        ...
        System.out.println("Missatge xifrat:" + xifraRotX(paraulesProba[3], numDesplaçat[3]));
        System.out.println("---------");
        ...
        for (int x = 0; x <= minuscules.length();x++){
            for(int i=0;i < cadenaXifrada.length();i++){
                ...
                if(Character.isLowerCase(c)){
                    for(int j = 0; j < mArray.length;j++){
                        if(c == mArray[j]){
                            c = mArray[(j - x + mArray.length) % mArray.length];
                            break;
                        }
                    }
                }
